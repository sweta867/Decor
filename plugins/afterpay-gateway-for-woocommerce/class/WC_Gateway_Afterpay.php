...
if (!class_exists('WC_Gateway_Afterpay')) {
	class WC_Gateway_Afterpay extends WC_Payment_Gateway
	{
		...
		private function init_user_agent() {
			...
			if ($this->settings['testmode'] == 'sandbox') {
				HTTP::addPlatformDetail('MPID', !empty($this->settings['test-mpid']) ? $this->settings['test-mpid'] : 'null');
			} else {
				HTTP::addPlatformDetail('MPID', !empty($this->settings['prod-mpid']) ? $this->settings['prod-mpid'] : 'null');
			}
			try {
		...
		public function filter_woocommerce_gateway_icon($icon_html, $gateway_id) {
			...
			$image_path = 'integration/checkout/logo-afterpay-colour-120x25';
