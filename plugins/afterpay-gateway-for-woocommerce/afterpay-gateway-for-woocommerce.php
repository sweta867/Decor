	...
	class Afterpay_Plugin
	{
		...
		public function init_website_assets()
		{
			...
			if ($instance->settings['enabled'] == 'yes') {
				...
				if ( $instance->get_api_env() == 'production' ) {
					wp_register_script( 'afterpay_js_lib', 'https://js.afterpay.com/afterpay-1.x.js', array(), $plugin_version, true );
					wp_register_script( 'afterpay_express_lib', "https://portal.afterpay.com/afterpay.js?merchant_key=woocommerce&plugin_version={$plugin_version}", array(), false, true );
				} else {
					wp_register_script( 'afterpay_js_lib', 'https://js.sandbox.afterpay.com/afterpay-1.x.js', array(), $plugin_version, true );
					wp_register_script( 'afterpay_express_lib', "https://portal.sandbox.afterpay.com/afterpay.js?merchant_key=woocommerce&plugin_version={$plugin_version}", array(), false, true );
				}
				...
				if (is_checkout()) {
					...
					wp_enqueue_script(
						...
						plugins_url('build/afterpay-page-checkout.js', __FILE__),
						['jquery', 'afterpay_express_lib'],
						$plugin_version,
						true
					);
				}
				...
				if(is_cart()) {
					wp_register_script( 'afterpay_express', plugins_url( 'build/afterpay-express.js', __FILE__ ), array('jquery', 'afterpay_express_lib'), $plugin_version, true );
		...
		public function init_admin_assets($hook)
		{
			if ( $hook == 'woocommerce_page_wc-settings' &&
				...
				wp_enqueue_script( 'afterpay_admin_js', plugins_url( 'build/afterpay-admin.js', __FILE__ ), array(), false, true );
