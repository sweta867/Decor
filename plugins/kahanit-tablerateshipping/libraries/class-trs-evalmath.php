...
/**
 * Class TRS_EvalMath. Supports basic math only (removed eval function).
 *
...
class TRS_EvalMath
{
    ...
    public function evaluate($expr)
    {
        ...
        if (preg_match('/^\s*([a-z]\w*)\s*=\s*(.+)$/', $expr, $matches)) {
            if (in_array($matches[1], $this->vb)) { // make sure we're not assigning to a constant
                return $this->trigger(sprintf(__('cannot assign to constant "%s"', 'tablerateshipping'), $matches[1]));
            }
            ...
            if (in_array($matches[1], $this->fb)) { // make sure it isn't built in
                return $this->trigger(sprintf(__('cannot redefine built-in function "%s()"', 'tablerateshipping'), $matches[1]));
            }
            ...
            for ($i = 0; $i < count($stack); $i++) { // freeze the state of the non-argument variables
                ...
                if (preg_match('/^[a-z]\w*$/', $token) and !in_array($token, $args)) {
                    if (array_key_exists($token, $this->v)) {
                        ...
                        return $this->trigger(sprintf(__('undefined variable "%s" in function definition', 'tablerateshipping'), $token));
                    }
                }
            }
    ...
    public function nfx($expr)
    {
        ...
        if (preg_match('/[^\%\w\s+*^\/()\.,-]/', $expr, $matches)) { // make sure the characters are all good
            return $this->trigger(sprintf(__('illegal character "%s"', 'tablerateshipping'), $matches[0]));
        }
        ...
        while (1) { // 1 Infinite Loop ;)
            ...
            if ($op == '-' and !$expecting_op) { // is it a negation instead of a minus?
                ...
                return $this->trigger(__('illegal character "_"', 'tablerateshipping')); // but not in the input expression
                //===============
                ...
                while (($o2 = $stack->pop()) != '(') { // pop off the stack back to the last (
                    if (is_null($o2)) {
                        return $this->trigger(__('unexpected ")"', 'tablerateshipping'));
                    } else {
                        $output[] = $o2;
                    }
                }
                if (preg_match("/^([A-Za-z]\w*)\($/", $stack->last(2), $matches)) { // did we just close a function?
                    ...
                    if (in_array($fnn, $this->fb)) { // check the argument count
                        if ($arg_count > 1) {
                            return $this->trigger(sprintf(__('too many arguments (%s given, 1 expected)', 'tablerateshipping'), $arg_count));
                        }
                    } elseif (array_key_exists($fnn, $this->f)) {
                        if ($arg_count != count($this->f[$fnn]['args'])) {
                            return $this->trigger(sprintf(__('wrong number of arguments (%s given, %s expected)', 'tablerateshipping'), $arg_count, count($this->f[$fnn]['args'])));
                        }
                    } else { // did we somehow push a non-function on the stack? this should never happen
                        return $this->trigger(__('internal error', 'tablerateshipping'));
                    }
                }
                $index++;
                //===============
                ...
                return $this->trigger(sprintf(__('unexpected operator "%s"', 'tablerateshipping'), $op));
            } else { // I don't even want to know what you did to get here
                return $this->trigger(__('an unexpected error occured', 'tablerateshipping'));
            }
            if ($index == strlen($expr)) {
                if (in_array($op, $ops)) { // did we end with an operator? bad.
                    return $this->trigger(sprintf(__('operator "%s" lacks operand', 'tablerateshipping'), $op));
                } else {
                    break;
                }
            }
        ...
        while (!is_null($op = $stack->pop())) { // pop everything off the stack and push onto output
            if ($op == '(') {
                return $this->trigger(__('expecting ")"', 'tablerateshipping'));
        ...
        foreach ($tokens as $token) { // nice and easy
            ...
            if (in_array($token, array('+', '-', '*', '/', '^', '%'))) {
                ...
                switch ($token) {
                    ...
                    case '/':
                        if ($op2 == 0) {
                            return $this->trigger(__('division by zero', 'tablerateshipping'));
                        }
                        $stack->push($op1 / $op2);
                        break;
                    case '^':
                ...
                if (is_numeric($token)) {
                    ...
                    return $this->trigger(sprintf(__('undefined variable "%s"', 'tablerateshipping'), $token));
                }
            }
        }
