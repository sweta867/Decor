...
/**
 * EH_Stripe_P24_Pay class.
 *
 * @extends EH_Stripe_Payment
 */
class EH_P24 extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->id                 = 'eh_p24_stripe';
        $this->method_title       = __( 'Przelewy24', 'payment-gateway-stripe-and-woocommerce-integration' );

        $url = add_query_arg( 'wc-api', 'wt_stripe', trailingslashit( get_home_url() ) );
        $this->method_description = sprintf( __( 'Przelewy24 is a Poland-based payment method aggregator that allows customers to complete transactions online using bank transfers. ' . '<a  class="thickbox" href="'.EH_STRIPE_MAIN_URL_PATH . 'assets/img/p24-preview.png?TB_iframe=true&width=100&height=100">[Preview] </a>', 'payment-gateway-stripe-and-woocommerce-integration' ));
        $this->supports = array(
            'products',
            'refunds',
        );
        ...
        $this->title                   = __($this->get_option( 'eh_stripe_p24_title' ), 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->description             = __($this->get_option( 'eh_stripe_p24_description' ), 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->enabled                 = $this->get_option( 'enabled' );
        $this->eh_order_button         = $this->get_option( 'eh_stripe_p24_order_button');
       ...
       add_action( 'woocommerce_api_eh_p24', array( $this, 'eh_p24_callback_handler' ) );
    }



    /**
     * Initialize form fields in p24 payment settings page.
     */
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'eh_p24_desc' => array(
                'type' => 'title',
                'description' => sprintf(__('%sSupported currencies: %sEUR, PLN%sStripe accounts in the following countries can accept the payment: %sAustralia, Austria, Belgium, Bulgaria, Canada, Cyprus, Czech Republic, Denmark, Estonia, Finland, France, Germany, Greece, Hong Kong, Hungary, Ireland, Italy, Japan, Latvia, Lithuania, Luxembourg, Malta, Mexico, Netherlands, New Zealand, Norway, Poland, Portugal, Romania, Singapore, Slovakia, Slovenia, Spain, Sweden, Switzerland, United Kingdom, United States%s %sRead documentation%s', 'payment-gateway-stripe-and-woocommerce-integration'), '<div class="wt_info_div"><ul><li>', '<b>', '</b></li><li>', '<b>', '</b></li></ul></div>', '<p><a target="_blank" href="https://www.webtoffee.com/woocommerce-stripe-payment-gateway-plugin-user-guide/#przelewy24">', '</a></p>'),
            ),
            'eh_stripe_p24_form_title'   => array(
                'type'        => 'title',
                'class'       => 'eh-css-class',
            ),
            'enabled'                       => array(
                'title'       => __('Przelewy24 Pay','payment-gateway-stripe-and-woocommerce-integration'),
                'label'       => __('Enable','payment-gateway-stripe-and-woocommerce-integration'),
                'type'        => 'checkbox',
                'default'     => isset($stripe_settings['eh_stripe_p24']) ? $stripe_settings['eh_stripe_p24'] : 'no',
                'desc_tip'    => __('Enables customers in the Single Euro Payments Area (Przelewy24) to pay by providing their bank account details.','payment-gateway-stripe-and-woocommerce-integration'),
            ),
            'eh_stripe_p24_title'         => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_p24_title']) ? $stripe_settings['eh_stripe_p24_title'] : __('Przelewy24 Pay', 'payment-gateway-stripe-and-woocommerce-integration'),
                'desc_tip'    => true,
            ),
            'eh_stripe_p24_description'     => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_p24_description']) ? $stripe_settings['eh_stripe_p24_description'] : __('Secure debit payment via Przelewy24.', 'payment-gateway-stripe-and-woocommerce-integration'),
                'desc_tip'    => true
            ),

            'eh_stripe_p24_order_button'    => array(
                ...
                'default'     => isset($stripe_settings['eh_stripe_p24_order_button']) ? $stripe_settings['eh_stripe_p24_order_button'] :__('Pay via Przelewy24', 'payment-gateway-stripe-and-woocommerce-integration'),
                'desc_tip'    => true
            )
        );   
    }
    ...
    public function get_icon() {
        ...
        $icon .= '<img src="' . WC_HTTPS::force_https_url(EH_STRIPE_MAIN_URL_PATH . 'assets/img/p24.svg') . '" alt="Przelewy24" width="52" title="Przelewy24" ' . $style . ' />';
    ...
    public function payment_fields() {
        ...
        if ($this->description) {
            echo apply_filters('eh_p24_desc', wpautop(wp_kses_post("<span>" . $this->description . "</span>")));
        }
        echo "</div>";
    ...
    public function elements_form() {
        ...
        <fieldset id="eh-<?php echo esc_attr( $this->id ); ?>-cc-form" class="eh-credit-card-form eh-payment-form" style="background:transparent;">
            ...
            <div class="eh-p24-errors" role="alert" style="color:#ff0000"></div>
            <div class="clear"></div>
        </fieldset>
        <?php
    }
    ...
    public function create_source( $order ) {
        ...
        $return_url            = add_query_arg('order_id', $order_id, WC()->api_request_url('EH_P24'));
           ...
           $source_response =  $this->create_source($order);
            ...
            if (isset($source_response->id) && !empty($source_response->id)) {
                ...
                if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
                    update_post_meta( $order_id, '_eh_p24_source_id', $source_id );
                } else {
                    $order->update_meta_data( '_eh_p24_source_id', $source_id );
                }
                $order->save();
                   ...
                   $customer = $this->create_stripe_customer($source_id, $order_id, ((WC()->version < '2.7.0') ? $order->billing_email : $order->get_billing_email()));
                    ...
                    if ($obj_stripe->is_subscription($order_id)) {
                        ...
                        if ( 0 >= $order->get_total() ) {
                            ...
                            EH_Helper_Class::wt_stripe_order_db_operations($order_id, $order, 'update', '_p24_customer_id', $customer->id, false); 
                    ...
                    if (!empty($customer)) {
                        $user_id = $order->get_user_id();
                        update_user_meta($user_id, "_p24_customer_id", $customer->id);
                    }
            ...
            $order->update_status( 'failed', sprintf( __( 'Przelewy24 payment failed: %s', 'payment-gateway-stripe-and-woocommerce-integration' ),$e->getMessage() ) );
    ...
    public function eh_make_charge_params( $order, $source_id,  $customer = null ) {
       ...
       if ( $customer ) {
        ...
        return apply_filters( 'eh_p24_generate_charge_request', $post_data, $order, $source_id );
    }
    ...
    public function eh_process_payment_response( $response, $order, $auto_redirect = true ) {
        ...
        if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
            update_post_meta( $order_id, '_eh_p24_charge_captured', $captured );
        } else {
            $order->update_meta_data( '_eh_p24_charge_captured', $captured );
        }
        ...
        if ( 'Captured' === $captured ) {
            ...
            $order->update_status( 'on-hold', sprintf( __( 'Stripe Przelewy24 order meta (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization.', 'payment-gateway-stripe-and-woocommerce-integration' ), $response->id) );
        }
    ...
    //webhook callback
    public function eh_p24_callback_handler() { 
       $order_id = (isset($_REQUEST['order_id']) && !empty($_REQUEST['order_id'])) ? $_REQUEST['order_id'] : '';
        ...
        try{ 
            ...
            $order->update_status( 'failed', sprintf( __( 'P24 payment failed: %s', 'payment-gateway-stripe-and-woocommerce-integration' ),$e->getMessage() ) );
            ...
            if (isset($source_response->error)) {
                ...
                if (isset($source_response->status) && !empty($source_response->status)) {
                    ...
                    elseif ('consumed' == $source_response->status) {
                     ...
                     throw new Exception( __('Unable to source status, please try again.', 'payment-gateway-stripe-and-woocommerce-integration'));
                }
            }
        }
    }
