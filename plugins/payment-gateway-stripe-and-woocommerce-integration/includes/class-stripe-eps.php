...
class EH_EPS extends WC_Payment_Gateway {
    ...
    public function payment_fields() {
        ...
        if ($this->description) {
            echo apply_filters('eh_eps_desc', wpautop(wp_kses_post("<span>" . $this->description . "</span>")));
        }
        echo "</div>";
    ...
    public function create_source( $order ) {
        ...
        $return_url            = add_query_arg('order_id', $order_id, WC()->api_request_url('EH_EPS'));
           ...
           $source_response =  $this->create_source($order);
            ...
            if (isset($source_response->id) && !empty($source_response->id)) {
                ...
                if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
                    update_post_meta( $order_id, '_eh_eps_source_id', $source_id );
                } else {
                    $order->update_meta_data( '_eh_eps_source_id', $source_id );
                }
                $order->save();
                ...
                if ($source_status == 'chargeable') {
                   $customer = $this->create_stripe_customer($source_id, $order_id, ((WC()->version < '2.7.0') ? $order->billing_email : $order->get_billing_email()));
                    ...
                    if ($obj_stripe->is_subscription($order_id)) {
                        ...
                        if ( 0 >= $order->get_total() ) {
                            $user_id = $order->get_user_id();
                             ...
                             EH_Helper_Class::wt_stripe_order_db_operations($order_id, $order, 'update', '_eps_customer_id', $customer->id, false); 
                    ...
                    if (!empty($customer)) {
                        $user_id = $order->get_user_id();
                        update_user_meta($user_id, "_eps_customer_id", $customer->id);
                    }
    ...
    public function eh_process_payment_response( $response, $order, $auto_redirect = true ) {
        ...
        if ( 'Captured' === $captured ) {
            ...
            $order->update_status( 'on-hold', sprintf( __( 'Stripe EPS order meta (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization.', 'payment-gateway-stripe-and-woocommerce-integration' ), $response->id) );
        }
            ...
            if (isset($source_response->error)) {
                ...
                if (isset($source_response->status) && !empty($source_response->status)) {
                    ...
                    elseif ('consumed' == $source_response->status) {
                     ...
                     throw new Exception( __('Unable find source status, please try again.', 'payment-gateway-stripe-and-woocommerce-integration'));
                }
            }
        }
    }
