 ...
 */
class EH_Sofort extends WC_Payment_Gateway {
    ...
    public function __construct() { 
        ...
        $this->id                 = 'eh_sofort_stripe';
        $this->method_title       = __( 'Sofort', 'payment_gateway_stripe_and_woocommerce_integration' );

        $this->method_description =  __( 'Sofort is a single use, delayed notification payment method that redirects customers to their bank’s portal to authenticate the payment.' . '<a  class="thickbox" href="'.EH_STRIPE_MAIN_URL_PATH . 'assets/img/sofort-preview.png?TB_iframe=true&width=100&height=100">[Preview] </a>', 'payment_gateway_stripe_and_woocommerce_integration' );
        $this->supports = array(
            'products',
            'refunds',
        );
        ...
        $this->title                   = __($this->get_option( 'eh_stripe_sofort_title' ), 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->description             = __($this->get_option( 'eh_stripe_sofort_description' ), 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->enabled                 = $this->get_option( 'enabled' );
        $this->eh_order_button         = $this->get_option( 'eh_stripe_sofort_order_button');
       ...
       add_action( 'woocommerce_api_eh_sofort', array( $this, 'eh_sofort_callback_handler' ) );
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            ...
            'eh_sofort_desc' => array(
                'type' => 'title',
                'description' => sprintf(__('%sSupported currency: %sEUR%s %sStripe accounts in the following countries can accept the payment: %sAustralia, Austria, Belgium, Bulgaria, Canada, Cyprus, Czech Republic, Denmark, Estonia, Finland, France, Germany, Greece, Hong Kong, Hungary, Ireland, Italy, Japan, Latvia, Lithuania, Luxembourg, Malta, Mexico, Netherlands, New Zealand, Norway, Poland, Portugal, Romania, Singapore, Slovakia, Slovenia, Spain, Sweden, Switzerland, United Kingdom, United States%s %s Read documentation %s', 'payment-gateway-stripe-and-woocommerce-integration'), '<div class="wt_info_div"><ul><li>', '<b>','</b>', '</li><li>', '<b>', '</b></li></ul></div>', '<p><a target="_blank" href="https://www.webtoffee.com/woocommerce-stripe-payment-gateway-plugin-user-guide/#sofort">', '</a></p>'),
            ),
            'eh_stripe_sofort_form_title'   => array(
                'type'        => 'title',
                'class'       => 'eh-css-class',
            ),
            'enabled'                       => array(
                'title'       => __('Sofort','payment_gateway_stripe_and_woocommerce_integration'),
                'label'       => __('Enable','payment_gateway_stripe_and_woocommerce_integration'),
                'type'        => 'checkbox',
                'default'     => isset($stripe_settings['eh_stripe_sofort']) ? $stripe_settings['eh_stripe_sofort'] : 'no',
                'desc_tip'    => __('Enables to accept payments using Sofort.','payment_gateway_stripe_and_woocommerce_integration'),
            ),

            'eh_stripe_sofort_title'         => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_sofort_title']) ? $stripe_settings['eh_stripe_sofort_title'] : __('Sofort', 'payment_gateway_stripe_and_woocommerce_integration'),
                'desc_tip'    => true,
            ),
            'eh_stripe_sofort_description'     => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_sofort_description']) ? $stripe_settings['eh_stripe_sofort_description'] : __('Accept payments using Sofort.', 'payment_gateway_stripe_and_woocommerce_integration'),
                'desc_tip'    => true
            ),
            'eh_stripe_sofort_lang-custom'     => array(
                'title'       => __('Customize language','payment_gateway_stripe_and_woocommerce_integration'),
                'type'        => 'checkbox',
                'label' => __('Enable to view the bank site in the required language.', 'payment_gateway_stripe_and_woocommerce_integration'),
                'default'     =>isset($stripe_settings['eh_stripe_sofort_lang']) ? $stripe_settings['eh_stripe_sofort_lang'] : 'no',
                'desc_tip'    => false
            ),
            'eh_stripe_sofort_language'     => array(
                'title'       => __('Preferred language','payment_gateway_stripe_and_woocommerce_integration'),
                'type'        => 'select',
                'description' => __('Select the required language for the bank site.', 'payment_gateway_stripe_and_woocommerce_integration'),
                'default' => 'en',
                'desc_tip' => true,
                'options' => array(
                    'en' => __('English', 'payment_gateway_stripe_and_woocommerce_integration'),
                    'de' => __('German', 'payment_gateway_stripe_and_woocommerce_integration'),
                    'it' => __('Italian', 'payment_gateway_stripe_and_woocommerce_integration'),
                    'fr' => __('French', 'payment_gateway_stripe_and_woocommerce_integration'),
                    'nl' => __('Dutch', 'payment_gateway_stripe_and_woocommerce_integration'),
                   // 'es' => __('Large', 'payment_gateway_stripe_and_woocommerce_integration')
                )                
            ),

            'eh_stripe_sofort_order_button'    => array(
                ...
                'default'     => isset($stripe_settings['eh_stripe_sofort_order_button']) ? $stripe_settings['eh_stripe_sofort_order_button'] :__('Pay via Sofort', 'payment_gateway_stripe_and_woocommerce_integration'),
                'desc_tip'    => true
            ),
            'eh_sofort_webhook_desc' => array(
                'type' => 'title',
                'description' => sprintf(__('%1$sTo accept payments via Sofort payment method, you must configure the webhook endpoint and subscribe to relevant events. %2$sClick here%3$s to know more%4$s', 'payment-gateway-stripe-and-woocommerce-integration'), '<div class="wt_info_div"><p>', '<a target="_blank" href="https://www.webtoffee.com/setting-up-webhooks-and-supported-webhooks/">', '</a>', '</p></div>'),
            ),
        );
    ...
    public function get_icon() {
        ...
        $icon .= '<img src="' . WC_HTTPS::force_https_url(EH_STRIPE_MAIN_URL_PATH . 'assets/img/sofort.svg') . '" width="55" alt="Sofort" title="Sofort" ' . $style . ' />';
    ...
    public function payment_scripts() {
        ...
        if ( (is_checkout()  && !is_order_received_page())) {
         ...
         wp_enqueue_script('eh_sofort_js', plugins_url('assets/js/eh-sofort.js', EH_STRIPE_MAIN_FILE), array('stripe_v3_js','jquery'),EH_STRIPE_VERSION, true);
            if (isset($stripe_settings['eh_stripe_mode']) && 'test' === $stripe_settings['eh_stripe_mode']) {
                if (!isset($stripe_settings['eh_stripe_test_publishable_key']) || !isset($stripe_settings['eh_stripe_test_secret_key']) || ! $stripe_settings['eh_stripe_test_publishable_key'] || ! $stripe_settings['eh_stripe_test_secret_key']) {
                    return false;
                }
           ...
           wp_localize_script('eh_sofort_js', 'eh_sofort_val', apply_filters('eh_sofort_val', $stripe_params));
        }
    }
    ...
    public function payment_fields() {
        ...
        echo "</div>";
        echo '<div class="eh-sofort-errors" role="alert" style="color:#ff0000"></div>';
    }
    ...
    public function process_payment($order_id) { 
        ...
        try{ 
            ...
            $payment_method = isset($_POST['eh_sofort_token']) ? sanitize_text_field($_POST['eh_sofort_token']) : '';
            ...
            if (!empty($customer) && isset($customer->id)) {
                ...
                update_user_meta($user_id, "_sofort_customer_id", sanitize_text_field($customer->id));
                ...
                if(! empty($intent)){
                    ...
                    if ( $intent->status === 'succeeded' ) {
                        wc_add_notice(__('An error has occurred internally, due to which you are not redirected to the order received page. Please contact support for more assistance.', 'payment_gateway_stripe_and_woocommerce_integration'), $notice_type = 'error');
            ...
            $order->update_status( 'failed', sprintf( __( 'Sofort payment failed: %s', 'payment_gateway_stripe_and_woocommerce_integration' ),$e->getMessage() ) );
    ...
    public function get_charge_details( $wc_order, $client, $currency, $amount, $payment_method) {
        ...
        $charge = array(
            'payment_method_types' => array('sofort'),
            'amount' => $amount,
        ...
        $eh_stripe = get_option("woocommerce_eh_sofort_stripe_settings");

        $product_list = implode(' | ', $product_name);

        $charge['metadata']['Products'] = substr($product_list, 0, 499);
        
        //prefferred language at sofort checkout
        if (isset($eh_stripe['eh_stripe_sofort_lang-custom']) && $eh_stripe['eh_stripe_sofort_lang-custom'] == 'yes') {
           $charge['payment_method_options']['sofort']['preferred_language'] = (isset($eh_stripe['eh_stripe_sofort_language']) ? $eh_stripe['eh_stripe_sofort_language'] : 'en' );
        }
        ...
        $charge['confirm'] =  true ;
        $charge['return_url'] =  add_query_arg('order_id', $order_id, WC()->api_request_url('EH_Sofort')) ; 
       ...
       return apply_filters('eh_sofort_payment_intent_args', $charge);
    }
    ...
    public function eh_sofort_callback_handler() {
    ...
    public function eh_process_payment_response( $response, $order = null ) {
        ...
        if (!empty($charge_response)) {
            ...
            if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
                update_post_meta( $order_id, '_eh_sofort_charge_captured', $captured );
            } else {
                $order->update_meta_data( '_eh_sofort_charge_captured', $captured );
            }
        }
        ...
        if($response->status == 'succeeded'){
            if ($charge_response->paid == true) {
                ...
                $order->add_order_note( __('Payment Status : ', 'payment_gateway_stripe_and_woocommerce_integration') . ucfirst($charge_response->status) .' [ ' . $order_time . ' ] . ' . __('Source : ', 'payment_gateway_stripe_and_woocommerce_integration') . $charge_response->payment_method_details->type . '. ' . __('Charge Status :', 'payment_gateway_stripe_and_woocommerce_integration') . $captured . (is_null($charge_response->balance_transaction) ? '' :'. Transaction ID : ' . $charge_response->balance_transaction) );
                WC()->cart->empty_cart();
