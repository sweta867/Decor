 ...
 */
class EH_iDEAL extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->id                 = 'eh_ideal_stripe';
        $this->method_title       = __( 'iDEAL', 'payment_gateway_stripe_and_woocommerce_integration' );

        $url = add_query_arg( 'wc-api', 'wt_stripe', trailingslashit( get_home_url() ) );
        $this->method_description = sprintf( __( 'iDEAL is a Netherlands-based payment method that allows customers to use bank credentials to complete transactions online. ' . '<a  class="thickbox" href="'.EH_STRIPE_MAIN_URL_PATH . 'assets/img/ideal-preview.png?TB_iframe=true&width=100&height=100">[Preview] </a>', 'payment_gateway_stripe_and_woocommerce_integration' ));
        $this->supports = array(
            'products',
            'refunds',
        ...
        $this->title                   = __($this->get_option( 'eh_stripe_ideal_title' ), 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->description             = __($this->get_option( 'eh_stripe_ideal_description' ), 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->enabled                 = $this->get_option( 'enabled' );
        $this->eh_order_button         = $this->get_option( 'eh_stripe_ideal_order_button');
        ...
        add_action( 'woocommerce_api_eh_ideal', array( $this, 'eh_ideal_callback_handler' ) );
        add_action( 'woocommerce_available_payment_gateways',array($this, 'eh_disable_ideal_gateway_for_order_pay' ));
    ...
    /**
     * Initialize form fields in ideal payment settings page.
     */
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            ...
            'enabled'                       => array(
                'title'       => __('iDEAL Pay','payment_gateway_stripe_and_woocommerce_integration'),
                'label'       => __('Enable','payment_gateway_stripe_and_woocommerce_integration'),
                'type'        => 'checkbox',
                'default'     => isset($stripe_settings['eh_stripe_ideal']) ? $stripe_settings['eh_stripe_ideal'] : 'no',
                'desc_tip'    => __('Enables customers in the Single Euro Payments Area (iDEAL) to pay by providing their bank account details.','payment_gateway_stripe_and_woocommerce_integration'),
            ),
            'eh_stripe_ideal_title'         => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_ideal_title']) ? $stripe_settings['eh_stripe_ideal_title'] : __('iDEAL Pay', 'payment_gateway_stripe_and_woocommerce_integration'),
                'desc_tip'    => true,
            ),
            'eh_stripe_ideal_description'     => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_ideal_description']) ? $stripe_settings['eh_stripe_ideal_description'] : __('Secure debit payment via iDEAL.', 'payment_gateway_stripe_and_woocommerce_integration'),
                'desc_tip'    => true
            ),

            'eh_stripe_ideal_order_button'    => array(
                ...
                'default'     => isset($stripe_settings['eh_stripe_ideal_order_button']) ? $stripe_settings['eh_stripe_ideal_order_button'] :__('Pay via iDEAL', 'payment_gateway_stripe_and_woocommerce_integration'),
                'desc_tip'    => true
            )
        );   
    }
    ...
    public function get_icon() {
        ...
        $icon .= '<img src="' . WC_HTTPS::force_https_url(EH_STRIPE_MAIN_URL_PATH . 'assets/img/ideal.svg') . '" alt="iDEAL" width="52" title="iDEAL" ' . $style . ' />';
    ...
    public function payment_scripts() {
        ...
        if ( (is_checkout()  && !is_order_received_page())) {
           ...
           wp_enqueue_script('eh_ideal_pay', plugins_url('assets/js/eh-ideal.js', EH_STRIPE_MAIN_FILE), array('stripe_v3_js','jquery'),EH_STRIPE_VERSION, true);
            if (isset($stripe_settings['eh_stripe_mode']) && 'test' === $stripe_settings['eh_stripe_mode']) {
                if (!isset($stripe_settings['eh_stripe_test_publishable_key']) || !isset($stripe_settings['eh_stripe_test_secret_key']) || ! $stripe_settings['eh_stripe_test_publishable_key'] || ! $stripe_settings['eh_stripe_test_secret_key']) {
                    return false;
                }
            ...
            $stripe_params['ideal_elements_option'] = apply_filters('eh_stripe_ideal_elements_option',array(
                    'style'  => array( 
                        'base' => array( 
                            'fontSize' => '15px' ,
                            "iconColor" => "#666EE8",
            ...
            wp_localize_script('eh_ideal_pay', 'eh_ideal_val', apply_filters('eh_ideal_val', $stripe_params));
        }
    }
    ...
    public function payment_fields() {
        ...
        if ($this->description) {
            echo apply_filters('eh_ideal_desc', wpautop(wp_kses_post("<span>" . $this->description . "</span>")));
        }
        echo "</div>";
        $pay_button_text = __('Pay', 'payment_gateway_stripe_and_woocommerce_integration');
        if (is_checkout_pay_page()) {
            $order_id = get_query_var('order-pay');
            $order = wc_get_order($order_id);
            $email = (WC()->version < '2.7.0') ? $order->billing_email : $order->get_billing_email();
            echo '<div
                id="eh-ideal-pay-data"
    ...
    public function elements_form() {
        ...
        <fieldset id="eh-<?php echo esc_attr( $this->id ); ?>-cc-form" class="eh-credit-card-form eh-payment-form" style="background:transparent;">
                ...
                <div class="form-row  form-row-wide">
                    ...
                    -->
                    <label for="eh-stripe-ideal-accountholder-name"><?php esc_html_e( 'Name', 'payment_gateway_stripe_and_woocommerce_integration' ); ?> <span class="required"></span></label>
               ...
               <div class="form-row  form-row-wide">
                    ...
                    when the customer clicks on the label.
                    -->
                    <label for="eh-stripe-ideal-element"><?php esc_html_e( 'iDEAL Bank', 'payment_gateway_stripe_and_woocommerce_integration' ); ?> <span class="required">*</span></label>
            ...
            <div class="eh-ideal-errors" role="alert" style="color:#ff0000"></div>
            <div class="clear"></div>
        </fieldset>
        <?php
    }
    ...
    public function process_payment($order_id) { 
        ...
        try{ 
            ...
            if (!empty($customer) && isset($customer->id)) {
                ...
                update_user_meta($user_id, "_ideal_customer_id", sanitize_text_field($customer->id));
                ...
                elseif (isset($intent->status) && ( $intent->status == 'requires_payment_method' )) {
                    return array(
                        'result'        => 'success',
            ...
            $order->update_status( 'failed', sprintf( __( 'iDEAL payment failed: %s', 'payment_gateway_stripe_and_woocommerce_integration' ),$e->getMessage() ) );
    ...
    public function get_charge_details( $wc_order, $client, $currency, $amount) {
        ...
        $charge = array(
            'payment_method_types' => array('ideal'),
            'amount' => $amount,
            'currency' => $currency,
            'metadata' => array(
                'order_id' => $wc_order->get_id(),
        ...
        //$charge['confirm'] =  true ;
        //$charge['return_url'] =  add_query_arg('order_id', $order_id, WC()->api_request_url('EH_iDEAL')) ; 
       ...
       return apply_filters('eh_ideal_payment_intent_args', $charge);
    }
    ...
    public function eh_ideal_callback_handler() {
        if (isset($_REQUEST['order_id']) && !empty($_REQUEST['order_id'])) {
            $order_id = $_REQUEST['order_id'];
            $order = wc_get_order( $order_id );

        }
        if (!empty($order) && isset($_REQUEST['payment_intent']) && !empty($_REQUEST['payment_intent'])) {
    ...
    public function eh_process_payment_response( $response, $order = null ) {
        ...
        if (!empty($charge_response)) {
            ...
            if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
                update_post_meta( $order_id, '_eh_ideal_charge_captured', $captured );
            } else {
                $order->update_meta_data( '_eh_ideal_charge_captured', $captured );
            }
        }
    ...
    public function modify_successful_payment_result( $result, $order_id ) {
        ...
        $verification_url = add_query_arg(
            array(
                'order_id'         => $order_id,
                '_wpnonce'      => wp_create_nonce( 'eh_stripe_acss_confirm_payment_intent' ),
                'redirect_to'   => rawurlencode( $result['redirect'] )
             ), WC()->api_request_url('EH_Ideal')
        );
         $redirect_url = sprintf( '#confirm-ideal-pi-%s:%s', $result['intent_secret'], rawurlencode( $verification_url ) );
        return array(
            'result'   => 'success',
     ...
     * @since 3.3.6
     */ 
    function eh_disable_ideal_gateway_for_order_pay( $available_gateways ) {
        if ( is_wc_endpoint_url( 'order-pay' ) ) {
            
           unset( $available_gateways['eh_ideal_stripe'] );
        }
