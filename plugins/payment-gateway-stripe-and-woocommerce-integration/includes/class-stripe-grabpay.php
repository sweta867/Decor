 ...
 */
class EH_Grabpay extends WC_Payment_Gateway {
    ...
    public function __construct() {
       ...
       add_action( 'woocommerce_api_eh_grabpay', array( $this, 'eh_grabpay_callback_handler' ) );
    }


    /**
     * Initialize form fields in grabpay payment settings page.
     */
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            ...
            'enabled'                       => array(
                'title'       => __('GrabPay','payment-gateway-stripe-and-woocommerce-integration'),
                'label'       => __('Enable','payment-gateway-stripe-and-woocommerce-integration'),
                'type'        => 'checkbox',
                'default'     => isset($stripe_settings['eh_stripe_grabpay']) ? $stripe_settings['eh_stripe_grabpay'] : 'no',
                'desc_tip'    => __('Enables customers in the Single Euro Payments Area (Grabpay) to pay by providing their bank account details.','payment-gateway-stripe-and-woocommerce-integration'),
            ),
            'eh_stripe_grabpay_title'         => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_grabpay_title']) ? $stripe_settings['eh_stripe_grabpay_title'] : __('GrabPay', 'payment-gateway-stripe-and-woocommerce-integration'),
                'desc_tip'    => true,
            ),
            'eh_stripe_grabpay_description'     => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_grabpay_description']) ? $stripe_settings['eh_stripe_grabpay_description'] : __('Secure debit payment via GrabPay.', 'payment-gateway-stripe-and-woocommerce-integration'),
                'desc_tip'    => true
            ),

            'eh_stripe_grabpay_order_button'    => array(
                ...
                'default'     => isset($stripe_settings['eh_stripe_grabpay_order_button']) ? $stripe_settings['eh_stripe_grabpay_order_button'] :__('Pay via GrabPay', 'payment-gateway-stripe-and-woocommerce-integration'),
                'desc_tip'    => true
            )
        );   
    }
    ...
    public function get_icon() {
        ...
        $icon .= '<img src="' . WC_HTTPS::force_https_url(EH_STRIPE_MAIN_URL_PATH . 'assets/img/grabpay.svg') . '" alt="GrabPay" width="52" title="GrabPay" ' . $style . ' />';
    ...
    public function payment_scripts() {
        ...
        if ( (is_checkout()  && !is_order_received_page())) {
           ...
           wp_enqueue_script('eh_grabpay_pay', plugins_url('assets/js/eh-grabpay.js', EH_STRIPE_MAIN_FILE), array('stripe_v3_js','jquery'),EH_STRIPE_VERSION, true);
            ...
            $stripe_params['grabpay_elements_option'] = apply_filters('eh_stripe_grabpay_elements_option',array(
                    'supportedCountries' => array( 'GrabPay' ),
            ...
            wp_localize_script('eh_grabpay_pay', 'eh_grabpay_val', apply_filters('eh_grabpay_val', $stripe_params));
        }
    }
    ...
    public function payment_fields() {
        ...
        if ($this->description) {
            echo apply_filters('eh_grabpay_desc', wpautop(wp_kses_post("<span>" . $this->description . "</span>")));
        }
        echo "</div>";
    ...
    public function elements_form() {
        ...
        <fieldset id="eh-<?php echo esc_attr( $this->id ); ?>-cc-form" class="eh-credit-card-form eh-payment-form" style="background:transparent;">
            ...
            <div class="eh-grabpay-errors" role="alert" style="color:#ff0000"></div>
            <div class="clear"></div>
        </fieldset>
        <?php
    }
    ...
    public function process_payment($order_id) { 
        ...
        try{ 
            ...
            if (!empty($customer) && isset($customer->id)) {
                ...
                update_user_meta($user_id, "_grabpay_customer_id", sanitize_text_field($customer->id));
    ...
    public function save_payment_intent_to_order( $order, $intent ) {
        ...
        if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
            update_post_meta( $order_id, 'eh_grabpay_intent_id', $intent->id );
        } else {
            $order->update_meta_data( 'eh_grabpay_intent_id', $intent->id );
        }
    ...
    public function get_charge_details( $wc_order, $client, $currency, $amount, $payment_method) {
        ...
        $charge = array(
            'payment_method_types' => array('grabpay'),
            'amount' => $amount,
        ...
        $charge['confirm'] =  true ;
        //$charge['capture_method'] = ($eh_stripe['eh_stripe_capture'] == 'yes') ? 'automatic' : 'manual';
        $charge['return_url'] =  add_query_arg('order_id', $order_id, WC()->api_request_url('EH_Grabpay')) ; 
    ...
    public function get_payment_intent_from_order( $order ) {
        ...
        if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
            $intent_id = get_post_meta( $order_id, 'eh_grabpay_intent_id', true );
        } else {
            $intent_id = $order->get_meta( 'eh_grabpay_intent_id' );
        }
    ...
    public function eh_process_payment_response( $response, $order = null ) {
        ...
        if (!empty($charge_response)) {
            ...
            if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
                update_post_meta( $order_id, '_eh_grabpay_charge_captured', $captured );
            } else {
                $order->update_meta_data( '_eh_grabpay_charge_captured', $captured );
            }
        }
