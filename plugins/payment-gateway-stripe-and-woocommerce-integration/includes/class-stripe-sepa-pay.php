                       ...
                       if (isset($decoded['data']['object']['metadata']['order_id']) && !empty($decoded['data']['object']['metadata']['order_id'])) {
                            ...
                            if(!$order = wc_get_order( $order_id )){
                                if(true === EH_Stripe_Payment::wt_stripe_is_HPOS_compatibile()){
                                     $meta = $wpdb->get_results( "SELECT order_id FROM " . $wpdb->prefix . "wc_orders_meta WHERE meta_key = '_transaction_id' AND meta_value= '" . $transaction_id  . "'" );
                                }
                                else{
                                ...
                                if (!empty($meta) && isset($meta[0]->post_id)) {
                                    ...
                                    if(true === EH_Stripe_Payment::wt_stripe_is_HPOS_compatibile()){
                                         $meta = $wpdb->get_results( "SELECT order_id FROM " . $wpdb->prefix . "wc_orders_meta WHERE meta_key = '_eh_stripe_payment_intent' AND meta_value= '" . $payment_intent_id  . "'" );
                                    }
                                    else{
                   ...
                   case 'charge.dispute.created':
                        if (isset($decoded['data']['object']['charge'])) {
                           $charge_id = sanitize_text_field($decoded['data']['object']['charge']);
                            if (!empty($charge_id)) {
                                if(true === EH_Stripe_Payment::wt_stripe_is_HPOS_compatibile()){
                                     $meta = $wpdb->get_results( "SELECT order_id FROM " . $wpdb->prefix . "wc_orders_meta WHERE meta_key = '_transaction_id' AND meta_value= '" . $charge_id  . "'" );
                                }
                                else{
                   ...
                   case 'payment_intent.payment_failed':
                        if (isset($decoded['data']['object']['id']) && !empty($decoded['data']['object']['id'])) {
                            ...
                            if (isset($decoded['data']['object']['metadata']['order_id']) && !empty($decoded['data']['object']['metadata']['order_id'])) {
                                ...
                                if(true === EH_Stripe_Payment::wt_stripe_is_HPOS_compatibile()){
                                     $meta = $wpdb->get_results( "SELECT order_id FROM " . $wpdb->prefix . "wc_orders_meta WHERE meta_key = '_eh_stripe_payment_intent' AND meta_value= '" . $intent_id  . "'" );
                                }
                                else{
