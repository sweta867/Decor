 ...
 */
class EH_Oxxo extends WC_Payment_Gateway {
    ...
    public function __construct() { 
        ...
        $this->id                 = 'eh_oxxo_stripe';
        $this->method_title       = __( 'OXXO', 'payment-gateway-stripe-and-woocommerce-integration' );

        $this->method_description =  __( 'OXXO allows customers to pay bills and online purchases in-store with cash.' . '<a  class="thickbox" href="'.EH_STRIPE_MAIN_URL_PATH . 'assets/img/oxxo-preview.png?TB_iframe=true&width=100&height=100">[Preview] </a>', 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->supports = array(
            'products',
            'refunds',
        );
        ...
        $this->title                   = __($this->get_option( 'eh_stripe_oxxo_title' ), 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->description             = __($this->get_option( 'eh_stripe_oxxo_description' ), 'payment-gateway-stripe-and-woocommerce-integration' );
        $this->enabled                 = $this->get_option( 'enabled' );
        $this->eh_order_button         = $this->get_option( 'eh_stripe_oxxo_order_button');
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'eh_oxxo_desc' => array(
                'type' => 'title',
                'description' => sprintf(__('%sSupported currency: %sMXN%sStripe accounts in the following countries can accept the payment: %sMexico%s', 'payment-gateway-stripe-and-woocommerce-integration'), '<div class="wt_info_div"><ul><li>', '<b>', '</b></li><li>', '<b>', '</b></li></ul></div>'),
            ),
            'eh_stripe_oxxo_form_title'   => array(
                'type'        => 'title',
                'class'       => 'eh-css-class',
            ),
            'enabled'                       => array(
                'title'       => __('OXXO','payment-gateway-stripe-and-woocommerce-integration'),
                'label'       => __('Enable','payment-gateway-stripe-and-woocommerce-integration'),
                'type'        => 'checkbox',
                'default'     => isset($stripe_settings['eh_stripe_oxxo']) ? $stripe_settings['eh_stripe_oxxo'] : 'no',
                'desc_tip'    => __('Enables to accept payments using OXXO.','payment-gateway-stripe-and-woocommerce-integration'),
            ),

            'eh_stripe_oxxo_title'         => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_oxxo_title']) ? $stripe_settings['eh_stripe_oxxo_title'] : __('OXXO', 'payment-gateway-stripe-and-woocommerce-integration'),
                'desc_tip'    => true,
            ),
            'eh_stripe_oxxo_description'     => array(
                ...
                'default'     =>isset($stripe_settings['eh_stripe_oxxo_description']) ? $stripe_settings['eh_stripe_oxxo_description'] : __('Accept payments using OXXO.', 'payment-gateway-stripe-and-woocommerce-integration'),
                'desc_tip'    => true
            ),

            'eh_stripe_oxxo_order_button'    => array(
                ...
                'default'     => isset($stripe_settings['eh_stripe_oxxo_order_button']) ? $stripe_settings['eh_stripe_oxxo_order_button'] :__('Pay via OXXO', 'payment-gateway-stripe-and-woocommerce-integration'),
                'desc_tip'    => true
            ),
            'eh_oxxo_webhook_desc' => array(
                'type' => 'title',
                'description' => sprintf(__('%1$sTo accept payments via OXXO payment method, you must configure the webhook endpoint and subscribe to relevant events. %2$sClick here%3$s to know more%4$s', 'payment-gateway-stripe-and-woocommerce-integration'), '<div class="wt_info_div"><p>', '<a target="_blank" href="https://www.webtoffee.com/setting-up-webhooks-and-supported-webhooks/">', '</a>', '</p></div>'),
            ),
        );
    ...
    public function get_icon() {
        ...
        //$icon .= '<img src="' . WC_HTTPS::force_https_url(EH_STRIPE_MAIN_URL_PATH . 'assets/img/oxxo.svg') . '" width="55" alt="Oxxo" title="Oxxo" ' . $style . ' />';
    ...
    public function payment_scripts() {
        ...
        if ( (is_checkout()  && !is_order_received_page())) {
         ...
         wp_enqueue_script('eh_oxxo_js', plugins_url('assets/js/eh-oxxo.js', EH_STRIPE_MAIN_FILE), array('stripe_v3_js','jquery'),EH_STRIPE_VERSION, true);
            if (isset($stripe_settings['eh_stripe_mode']) && 'test' === $stripe_settings['eh_stripe_mode']) {
                if (!isset($stripe_settings['eh_stripe_test_publishable_key']) || !isset($stripe_settings['eh_stripe_test_secret_key']) || ! $stripe_settings['eh_stripe_test_publishable_key'] || ! $stripe_settings['eh_stripe_test_secret_key']) {
                    return false;
                }
    ...
    public function payment_fields() {
        ...
        <fieldset id="eh-<?php echo esc_attr( $this->id ); ?>-cc-form" class="eh-credit-card-form eh-payment-form" style="background:transparent;">  
            ...
            <div class="eh-oxxo-errors" role="alert" style="color:#ff0000"></div>
            <div class="clear"></div>
        </fieldset><?php
    }
    ...
    public function process_payment($order_id) { 
        ...
        try{ 
            ...
            if (!empty($customer) && isset($customer->id)) {
                ...
                update_user_meta($user_id, "_oxxo_customer_id", sanitize_text_field($customer->id));
                ...
                elseif (isset($intent->status) && ( $intent->status == 'requires_action' ) &&
                    $intent->next_action->type == 'oxxo_display_details') {

                    return array(
                        'result'        => 'success',
                        'redirect'      => $intent->next_action->oxxo_display_details->hosted_voucher_url,
                    );
                } else {
            ...
            $order->update_status( 'failed', sprintf( __( 'OXXO payment failed: %s', 'payment-gateway-stripe-and-woocommerce-integration' ),$e->getMessage() ) );
    ...
    public function get_charge_details( $wc_order, $client, $currency, $amount) {
        ...
        $charge = array(
            'payment_method_types' => array('oxxo'),
            'amount' => $amount,
        ...
        $eh_stripe = get_option("woocommerce_eh_oxxo_stripe_settings");

        $product_list = implode(' | ', $product_name);

        $charge['metadata']['Products'] = substr($product_list, 0, 499);
        
        //prefferred language at oxxo checkout
        /*if (isset($eh_stripe['eh_stripe_oxxo_lang-custom']) && $eh_stripe['eh_stripe_oxxo_lang-custom'] == 'yes') {
           $charge['payment_method_options']['oxxo']['preferred_language'] = (isset($eh_stripe['eh_stripe_oxxo_language']) ? $eh_stripe['eh_stripe_oxxo_language'] : 'en' );
        }*/
        ...
        //$charge['confirm'] =  true ;
        //$charge['return_url'] =  add_query_arg('order_id', $order_id, WC()->api_request_url('EH_Oxxo')) ; 
       ...
       return apply_filters('eh_oxxo_payment_intent_args', $charge);
    }
    ...
    public function eh_oxxo_callback_handler() {
    ...
    public function eh_process_payment_response( $response, $order = null ) {
        ...
        if (!empty($charge_response)) {
            ...
            if ( version_compare(WC_VERSION, '2.7.0', '<') ) {
                update_post_meta( $order_id, '_eh_oxxo_charge_captured', $captured );
            } else {
                $order->update_meta_data( '_eh_oxxo_charge_captured', $captured );
            }
        }
    ...
    public function modify_successful_payment_result( $result, $order_id ) {
        ...
        $verification_url = add_query_arg(
         ...
         $redirect_url = sprintf( '#confirm-oxxo-pi-%s:%s', $result['intent_secret'], rawurlencode( $verification_url ) );
        return array(
            'result'   => 'success',
    ...
    function eh_disable_gateway_for_order_pay( $available_gateways ) {
        if ( is_wc_endpoint_url( 'order-pay' ) ) {
           ...
           unset( $available_gateways['eh_oxxo_stripe'] );
        }
